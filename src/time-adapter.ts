...
import ITimeOptions from './i-time-options';
...
export class TimeAdapter {
	...
	static parseTime(time: string, opts: ITimeOptions): DateTime {
	...
	static formatTime(
		...
		const { format } = opts;
		const parsedTime = TimeAdapter.parseTime(
			time,
			opts
	...
	static toLocaleTimeString(
		...
		const timeFormat = { ...DateTime.TIME_SIMPLE, hourCycle } as any;
	...
	private static getLocaleOptionsByTime(
		...
		const { numberingSystem, locale } = DateTime.local().setLocale(opts.locale as string).resolvedLocaleOpts();
